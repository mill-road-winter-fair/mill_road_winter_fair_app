...
class _TimetablePageState extends State<TimetablePage> {
  ...
  late double
  _dayPixelsPerMinute; // scale for the current day view, whatever its orientation
  ...
  String _searchQuery = '';
  bool _isSearching =
      false; // true when the search bar is open (with/without text)
  static const leftColumnWidth =
      44.0; // how much space to leave for the time labels
  ...
  late Map<String, List<PositionedEvent>>
  thePreparedEvents; // read from the listings
  late Map<String, List<PositionedEvent>>
  theFilteredEvents; // filtered from the above based on onlyNowOrSoon
  ...
  void _scheduleSearchAnalytics(String searchTerm) {
    ...
    _searchAnalyticsTimer = Timer(const Duration(milliseconds: 750), () {
      widget.analyticsService.logSearch(
        trimmedSearchTerm,
        searchArea: 'timetable',
      );
    });
  }
  ...
  void didChangeDependencies() {
    ...
    if (_deviceOrientationSaved != null &&
        currentOrientation != _deviceOrientationSaved) {
      ...
      if (currentOrientation == Orientation.landscape) {
        _dayPixelsPerMinute =
            pixelsPerMinuteL; // the local version which may have changed from that in schedule
      } else {
        _dayPixelsPerMinute =
            pixelsPerMinuteP; // the local version which may have changed from that in schedule
      }
  ...
  void loadScales() async {
    ...
    setState(
      () => loading = false,
    ); // this should be the last instruction of the last part of async initialisations
  }
  ...
  void calculateInitialScalesIfNeeded() async {
    if (pixelsPerMinuteP == 0 || pixelsPerMinuteL == 0) {
      debugPrint(
        '_TimetablePageState calculateInitialScalesIfNeeded estimating initial scale',
      );
  ...
  void startClockUpdates(VoidCallback tick) {
    ...
    final initialDelay =
        delay - Duration(seconds: now.second, milliseconds: now.millisecond);
    _nowLineTimer?.cancel();
    ...
    debugPrint(
      '_TimetablePageState startClockUpdates started initially $initialDelay then every $delay for function $tick',
    );
  }
  ...
  Map<String, List<PositionedEvent>> prepareEvents(
    List<Map<String, dynamic>> theEvents,
  ) {
    ...
    for (var ev in theEvents) {
      ...
      if (((ev['performanceMusic'] != null &&
                  ev['performanceMusic'] == 'TRUE') ||
              (ev['performanceChildrens'] != null &&
                  ev['performanceChildrens'] == 'TRUE') ||
              (ev['performanceDance'] != null &&
                  ev['performanceDance'] == 'TRUE') ||
              (ev['performanceOther'] != null &&
                  ev['performanceOther'] == 'TRUE') ||
              (ev['visitExperience'] != null &&
                  ev['visitExperience'] == 'TRUE')) &&
  ...
  Map<String, List<PositionedEvent>> filterEventsAndComputeDefaults(
    Map<String, List<PositionedEvent>> theEvents,
    bool onlyNowOrSoon,
    bool? filteredMusicOrNot,
    String searchQuery,
  ) {
    debugPrint(
    ...
    for (final location in theEvents.entries) {
      ...
      for (int i = 0; i < theEventsAtThisLocation.length; i++) {
        ...
        if (!ev.cancelled &&
            (!onlyNowOrSoon ||
                (ev.startTime.isBefore(now) &&
                    ev.endTime.difference(now).inMinutes >= -5) ||
                (ev.startTime.isAfter(now) &&
                    (ev.startTime.difference(now).inMinutes <= 30))) &&
            (searchQuery == '' ||
                ev.name.toString().toLowerCase().contains(_searchQuery)) &&
            (filteredMusicOrNot == null ||
                (filteredMusicOrNot == true && ev.isMusic) ||
                (filteredMusicOrNot == false && !ev.isMusic))) {
  ...
  static List<PositionedEvent> computeLanes(
    ...
    for (var ev in events) {
      ...
      final height = max(
        12.0,
        ev.endTime.difference(ev.startTime).inMinutes * pxPerMin,
      );
      out.add(
        PositionedEvent(
  ...
  Widget eventRect(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final maxTitleFontSize = max(
          minTitleFontSize,
          snap(rawMaxTitleFontSize, minTitleFontSize),
        );
        final maxLines =
            (pe.height / (minTitleFontSize * 1.1)).floor() -
            (includeDate ? 1 : 0);

        final rawMaxTimeFontSize = (9.0 + 0.7 * (pe.width / 20)).clamp(
          8.0,
          13.0,
        );
        final maxTimeFontSize = max(
          minTimeFontSize,
          snap(rawMaxTimeFontSize, minTimeFontSize),
        );
        ...
        return SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              if (includeDate)
                SizedBox(height: (pe.height * 0.05).clamp(2, pe.height * 0.25)),
              if (includeDate)
                Container(
  ...
  void scrollToKey(GlobalKey theKey, double alignment) async {
    debugPrint(
      '_TimetablePageState scrollToKey called with theKey=$theKey alignment=$alignment',
    );
  ...
  void _toggleOnlyNowOrSoon() {
    ...
    widget.analyticsService.logPreferenceSet(
      'timetable_now_or_soon',
      newonlyNowOrSoon.toString(),
    );
    ...
    Fluttertoast.showToast(
      msg:
          (newonlyNowOrSoon)
              ? 'Only showing what’s on now or starting soon'
              : 'Showing everything',
  ...
  void _toggleFilteredMusicOrNot() {
    debugPrint(
      '_TimetablePageState _toggleFilteredMusicOrNot called with widget.filteredMusicOrNot=$widget.filteredMusicOrNot',
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.onlyNowOrSoon != _onlyNowOrSoonSaved ||
        widget.filteredMusicOrNot != _filteredMusicOrNotSaved) {
      ...
      theFilteredEvents = filterEventsAndComputeDefaults(
        thePreparedEvents,
        widget.onlyNowOrSoon,
        widget.filteredMusicOrNot,
        _searchQuery,
      );
    }
        ...
        IconButton(
          ...
          onLongPress:
              () => showMiniPopup(
                context,
                subcategoryIconKey,
                'Tap to switch between showing just music, everything but music, or everything',
                analyticsService: widget.analyticsService,
              ),
          onPressed: () {
            HapticFeedback.lightImpact();
            widget.analyticsService.logButtonTapped(
              'timetable_category_filter',
            );
        ...
        IconButton(
          ...
          onLongPress:
              () => showMiniPopup(
                context,
                nowOrSoonIconKey,
                'Tap to switch between showing everything and showing just what’s on now or starting soon',
                analyticsService: widget.analyticsService,
              ),
          onPressed: () {
            HapticFeedback.lightImpact();
            widget.analyticsService.logButtonTapped(
              'timetable_now_or_soon_toggle',
            );
            (isItEventDay())
                ...
                : showMiniPopup(
                  context,
                  nowOrSoonIconKey,
                  '‘Now or soon’ is only available when the Fair is underway',
                  fgColour: colorScheme.error,
                  analyticsService: widget.analyticsService,
                );
          },
          icon: Icon(
            ...
            color:
                (isItEventDay())
                    ...
                    : appBarTheme.foregroundColor!.withAlpha(130),
          ),
        ),
        IconButton(
          key: searchIconKey,
          ...
          onLongPress:
              () => showMiniPopup(
                ...
                searchIconKey,
                (_isSearching)
                    ? 'Tap to close the search bar and cancel your search'
                    : 'Tap to open the search bar',
                analyticsService: widget.analyticsService,
              ),
          onPressed: () {
            HapticFeedback.lightImpact();
            ...
            builder: (context, constraints) {
              totalWidth =
                  max(
                    ...
                    theFilteredEvents.length * 100.0 + leftColumnWidth,
                  ) -
              ...
              if (!scaling) {
                ...
                if (widget.onlyNowOrSoon) {
                  _dayPixelsPerMinute = max(
                    0.8,
                    min(
                      20,
                      (constraints.maxHeight - 48) / max(spanMinutes, 30),
                    ),
                  );
                } else {
                  if (currentOrientation == Orientation.landscape) {
                ...
                swimlanes.add(
                  Positioned(
                    ...
                    left:
                        12 +
                        leftColumnWidth +
                        i * (totalWidth - leftColumnWidth) / cols,
                    height: spanMinutes * _dayPixelsPerMinute + 8,
                    child: Container(
                      color: colorScheme.surfaceDim,
                      width: (totalWidth - leftColumnWidth) / cols - 22,
                    ),
                  ),
                );
              }
              ...
              while (t.isBefore(
                timelineMaxEnd.add(Duration(minutes: markInterval)),
              )) {
                final top = max(
                  0.0,
                  t.difference(timelineMinStart).inMinutes *
                      _dayPixelsPerMinute,
                );
                final timeLabel =
              ...
              final nowTop =
                  max(
                    0.0,
                    (now.difference(timelineMinStart).inMinutes) *
                        _dayPixelsPerMinute,
                  ) -
                  1.5;
              final timelineHeight = max(
                constraints.maxHeight - 40,
                spanMinutes * _dayPixelsPerMinute + 4,
              );
                                      ...
                                      IconButton(
                                        ...
                                        onPressed: () {
                                          ...
                                          setState(() {
                                            ...
                                            _searchController.clear();
                                            theFilteredEvents =
                                                filterEventsAndComputeDefaults(
                                                  thePreparedEvents,
                                                  widget.onlyNowOrSoon,
                                                  widget.filteredMusicOrNot,
                                                  _searchQuery,
                                  ...
                                  child: GestureDetector(
                                    ...
                                    onScaleUpdate: (details) {
                                      ...
                                      final newdayPixelsPerMinute = max(
                                        ...
                                        min(
                                          1.5,
                                          startPixelsPerMinute * dampenedScale,
                                        ),
                                      );
                                      if (newdayPixelsPerMinute !=
                                          _dayPixelsPerMinute) {
                                        setState(() {
                                          _dayPixelsPerMinute = max(
                                            ((constraints.maxHeight - 40) /
                                    ...
                                    onScaleEnd: (_) async {
                                      if (scaling) {
                                        ...
                                        if (!widget.onlyNowOrSoon) {
                                          ...
                                          if (MediaQuery.orientationOf(
                                                context,
                                              ) ==
                                              Orientation.landscape) {
                                            if (_dayPixelsPerMinute !=
                                                pixelsPerMinuteL) {
                                              // only saved if genuinely changed
                                              widget.analyticsService
                                                  .logPreferenceSet(
                                                    'timetable_scale_landscape',
                                                    _dayPixelsPerMinute
                                                        .toStringAsFixed(2),
                                                  );
                                              pixelsPerMinuteL =
                                      ...
                                      child: SizedBox(
                                        ...
                                        child: Stack(
                                          children: [
                                            ...
                                            ...swimlanes,
                                            // red 'now' line
                                            if (timelineMinStart.isBefore(
                                                  DateTime.now(),
                                                ) &&
                                                timelineMaxEnd.isAfter(
                                                  DateTime.now(),
                                                ))
                                              Positioned(
                                                key: nowLineKey,
                                                                  ...
                                                                  : Positioned(
                                                                    ...
                                                                    child: GestureDetector(
                                                                      ...
                                                                      child: Container(
                                                                        ...
                                                                        decoration: BoxDecoration(
                                                                          color:
                                                                              (favouriteListingKeys.value.contains(
                                                                                    pe.id,
                                                                                  ))
