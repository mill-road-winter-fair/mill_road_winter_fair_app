...
class FilteredListingsPageState extends State<FilteredListingsPage> {
  ...
  String _searchQuery = '';
  List<bool> detailsVisibilityList = List<bool>.filled(
    500,
    false,
  ); // start with plenty enough to load all listings
  int firstNextListingIndex =
      -1; // the first listing that hasn't passed its end time, when sorted by start time
  int numberOfVisibleListings = -1;
  late String filterCategory;
  bool isShowingJustPerformance =
      false; // when selected subcategory starts with 'Performance'
  ...
  String calculateAppBarTitle() {
    ...
    if (filterCategory == 'favourite') {
      appBarTitle +=
          (appBarTitle.isEmpty)
              ? 'Favourite listings'
              : ' favourites'; // 'filtered favourite listings' doesn't fit!
    } else {
  ...
  List<Map<String, dynamic>> _applySorting(
    ...
    try {
      ...
      if ((preferredSortingMethod == SortingMethod.startTime &&
          !(isShowingJustPerformance || filterCategory == 'favourite'))) {
      ...
      if (preferredSortingMethod == SortingMethod.alphabetical ||
          useFallbackSorting == true) {
        ...
        allListings.sort((a, b) {
          final distanceCompare = a['approximateDistanceMetres'].compareTo(
            b['approximateDistanceMetres'],
          );
        ...
        if (currentLatLng != null) {
          ...
          allListings.sort((a, b) {
            ...
            return timeCompare != 0
                ? timeCompare
                : a['approximateDistanceMetres'].compareTo(
                  b['approximateDistanceMetres'],
                );
          });
        } else {
    ...
    if (selectedValue is SortingMethod) {
      if (selectedValue == SortingMethod.nearest && currentLatLng == null) {
        Fluttertoast.showToast(
          msg:
              'Location services and permissions are required to determine distances',
  ...
  Widget build(BuildContext context) {
    debugPrint(
      'FilteredListingsPageState build() called with filterCategory=$filterCategory and subfilterCategory=${widget.subfilterCategory}',
    );
    ...
    isShowingJustPerformance =
        (widget.subfilterCategory != null &&
            widget.subfilterCategory!.length > 11 &&
            widget.subfilterCategory!.substring(0, 11) == 'performance');
    ...
    if (filterCategory == 'all' || filterCategory == '') {
      categoryFiltered =
          listings
              .where((listing) => listing['groupParent'] == 'FALSE')
              .toList();
    ...
    if (widget.subfilterCategory == null) {
      ...
      subCategoryFiltered =
          categoryFiltered
              .where((listing) => listing[widget.subfilterCategory] == 'TRUE')
              .toList();
    }
    ...
    if (_hidePastListings) {
      numberOfVisibleListings =
          filteredListings
              .where((listing) => !hasEventEnded(listing['endTime']))
              .length;
    } else {
      numberOfVisibleListings = filteredListings.length;
    }
    int?
    firstVisibleIndex; // will be used to store the first listing that is actually visible
          ...
          IconButton(
            ...
            onLongPress:
                () => showMiniPopup(
                  context,
                  nowOrSoonIconKey,
                  'Tap to scroll to now to see what’s on or starting soon',
            ...
            onPressed: () {
              ...
              if (isItEventDay()) {
                if (firstNextListingIndex < 0) {
                  ...
                  List<Map<String, dynamic>> sortedListingsTemp = _applySorting(
                    subCategoryFiltered,
                  );
                  List<Map<String, dynamic>> filteredListingsTemp =
                      _applySearchFilter(sortedListingsTemp);
                  firstNextListingIndex = findFirstNextListingIndex(
                    filteredListingsTemp,
                  );
                  if (firstNextListingIndex < 0) {
                    preferredSortingMethod =
                        savedSortingMethod; // restore if not found a listing to scroll to
                  } else {
                    ...
                    if (_hidePastListings) {
                      numberOfVisibleListings =
                          filteredListings
                              .where(
                                (listing) => !hasEventEnded(listing['endTime']),
                              )
                              .length;
                    } else {
                      numberOfVisibleListings = filteredListings.length;
                ...
                showMiniPopup(
                  context,
                  nowOrSoonIconKey,
                  '‘Scroll to now’ is only available when the Fair is underway',
          ...
          IconButton(
            ...
            onLongPress:
                () => showMiniPopup(
                  ...
                  hidePastIconKey,
                  (_hidePastListings)
                      ? 'Tap to show all events and performances'
                      : 'Tap to hide events and performances that have passed',
                  analyticsService: widget.analyticsService,
                ),
            onPressed: () {
              HapticFeedback.lightImpact();
              widget.analyticsService.logButtonTapped(
                'listings_hide_past_toggle',
              );
              if (isItEventDay()) {
                setState(() {
                ...
                Fluttertoast.showToast(
                  msg:
                      (_hidePastListings)
                          ? 'Hiding all events and performances that have passed'
                          : 'Showing all events and performances',
                ...
                showMiniPopup(
                  context,
                  hidePastIconKey,
                  '‘Hide past listings’ is only available when the Fair is underway',
                  fgColour: colorScheme.error,
                  analyticsService: widget.analyticsService,
                );
              }
                                ...
                                ConstrainedBox(
                                  ...
                                  child: SearchBar(
                                    ...
                                    hintText: switch (filterCategory) {
                                      ...
                                      'food' =>
                                        'Search food & drink vendors...',
                                      'shopping' => 'Search market stalls...',
                                      'performanceMusic' =>
                                        'Search musical performances...',
                                      'performanceChildrens' =>
                                        'Search children’s activities...',
                                      'performanceDance' =>
                                        'Search dance performances...',
                                      'performanceOther' =>
                                        'Search other performances...',
                                      'charityCommunityInfo' =>
                                        'Search charity, community & info...',
                                      'visitExperience' =>
                                        'Search visits & experiences...',
                                      'service' => 'Search services...',
                                      'favourite' =>
                                        'Search favourite listings...',
                                      _ => 'Search listings...',
                                    },
                            ...
                            NotificationListener<ScrollNotification>(
                              ...
                              child: ScrollablePositionedList.builder(
                                ...
                                itemBuilder: (context, index) {
                                  final listing =
                                      filteredListings[index]; // since index=0 is the sort/search bar
                                  final approximateDistanceMetres =
                                      listing['approximateDistanceMetres'] ?? 0;
                                  final approximateDistance =
                                      '(approx. ${convertDistanceUnits(approximateDistanceMetres, preferredDistanceUnits)})';
                                  LatLng destinationLatLng = stringToLatLng(
                                    listing['latLng'],
                                  );
                                  if (!_hidePastListings ||
                                      !hasEventEnded(listing['endTime'])) {
                            ...
                            ValueListenableBuilder<Iterable<ItemPosition>>(
                              ...
                              builder: (context, positions, _) {
                                if (positions.isEmpty ||
                                    firstVisibleIndex == null ||
                                    numberOfVisibleListings == 0) {
                                  return const SizedBox.shrink();
                                }
                                final visiblePositions = positions.where((p) {
                                  return p.index >= firstVisibleIndex! &&
                                      p.index <
                                          firstVisibleIndex! +
                                              numberOfVisibleListings;
                                });
                                ...
                                final indices = visiblePositions.map(
                                  (p) => p.index,
                                );
                                final minIndex = indices.reduce(
                                  (a, b) => a < b ? a : b,
                                );
                                final maxIndex = indices.reduce(
                                  (a, b) => a > b ? a : b,
                                );
                                final minIndexRelative =
                                    minIndex - firstVisibleIndex!;
                                final visibleFraction = ((maxIndex -
                                            minIndex +
                                            1) /
                                        numberOfVisibleListings)
                                    .clamp(0.02, 1.0);
                                      ...
                                      child: AnimatedOpacity(
                                        ...
                                        child: GestureDetector(
                                          ...
                                          onVerticalDragUpdate: (details) {
                                            _showThumb();
                                            final localDy = details
                                                .localPosition
                                                .dy
                                                .clamp(0.0, trackHeight);
                                            final fraction = (localDy /
                                                    trackHeight)
                                                .clamp(0.0, 1.0);
                                            final targetIndex =
                                                (fraction *
