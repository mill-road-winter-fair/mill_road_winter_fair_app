...
class MapPageState extends State<MapPage> {
  ...
  _moveCameraToBoundsWithRotation(LatLng southwestMin, LatLng northeastMax,
    ...
    if (mapWidth != null && mapHeight != null) {
      theZoom = zoomForBounds(
          southwestMin, northeastMax, Size(mapWidth!, mapHeight!),
          padding: padding);
    } else {
      theZoom = 15;
  ...
  double zoomForBounds(
    ...
    final usableWidth = mapSize.width -
        (2 * padding * 4 / MediaQuery.of(context).devicePixelRatio);
    final usableHeight = mapSize.height -
        (2 * padding * 4 / MediaQuery.of(context).devicePixelRatio);

    if (usableWidth <= 0 || usableHeight <= 0) return 0;

    final latFraction =
        (_latRad(northeastMax.latitude) - _latRad(southwestMin.latitude)) / pi;
...
//    final lngZoom = log(usableWidth / WORLD_DIM / lngFraction) / ln2;
