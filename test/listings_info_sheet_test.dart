...
void main() {
  ...
  testWidgets('full page favourite updates and directions returns to source',
    ...
    final directionsButton = find.widgetWithText(ElevatedButton, 'Directions');
    expect(
        tester
            .widget<ElevatedButton>(favouriteButton)
            .style
            ?.backgroundColor
            ?.resolve({}),
        tester
            .widget<ElevatedButton>(directionsButton)
            .style
            ?.backgroundColor
            ?.resolve({}));
    expect(
  ...
  testWidgets('image URL is loaded and failure leaves details usable',
    ...
    expect(
        ...
        greaterThan(tester
            ...
            .dy));
    expect(
        imageRect.bottom, lessThan(tester.getTopLeft(find.text('Website')).dy));
  ...
  for (final theme in ['light', 'dark', 'highContrast']) {
    testWidgets('missing image and long text fit narrow screen in $theme theme',
      ...
      final firstButtonTop = tester.getTopLeft(actionButtons.at(0)).dy;
      expect(tester.getTopLeft(actionButtons.at(1)).dy, firstButtonTop);
      expect(tester.getTopLeft(actionButtons.at(2)).dy, firstButtonTop);
