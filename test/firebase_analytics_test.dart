...
void main() {
  ...
  for (final consent in [null, false]) {
    test(
      'all event families and properties are suppressed for consent=$consent',
      () async {
  ...
  test(
    'startup leaves unanswered consent unset and disables collection',
    () async {
  ...
  test(
    'saved opt-in restores properties without fabricating changes or an Unknown screen',
    () async {
  ...
  test(
    'opt-in persists consent and synchronizes preferences before tracking current screen',
    () async {
      ...
      expect(sdk.events.last['parameters'], {
        'button_id': 'theme_preference_option',
  ...
  test('revoking consent stops all further events and properties', () async {
    ...
    expect(sdk.properties, isEmpty);
    expect(
      (await SharedPreferences.getInstance()).getBool('usageAnalyticsEnabled'),
      false,
    );
  });

  test(
    'listing button events identify same-named listings without leaking context into other buttons',
    () async {
      usageAnalyticsEnabled = true;
      ...
      expect(sdk.events[0]['parameters'], {
        'button_id': 'visit_listing_website',
        'screen_name': 'ListingsPage',
  ...
  test('analytics backend failures do not escape into UI callbacks', () async {
    ...
    await expectLater(
      service.logPreferenceSet('sorting_method', 'nearest'),
      completes,
    );
  });
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Builder(
          builder:
              (context) => TextButton(
                onPressed: () => service.showAnalyticsConsentDialog(context),
