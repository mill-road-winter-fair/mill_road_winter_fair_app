...
class RecordingAnalyticsService extends FakeAnalyticsService {
  ...
  Future<void> logThemePreferenceSet(String theme) async =>
      calls.add('theme:$theme');
  @override
  Future<void> logDistanceUnitPreferenceSet(String unit) async =>
      calls.add('unit:$unit');
  @override
  Future<void> showAnalyticsConsentDialog(BuildContext context) async =>
      calls.add('consent_prompt');
}
...
void main() {
  ...
  setUp(() async {
    ...
    await loadSettings();
    firstExecution =
        true; // Avoid the listing reminder's unrelated toast timer.
    listings = [
      {
        'id': '1',
        'visibleOnMap': 'TRUE',
  ...
  testWidgets(
    'navigation logs once after haptics and before invoking the callback',
    (tester) async {
  ...
  for (final width in [350.0, 500.0]) {
    testWidgets('Details logs once and invokes the callback at width $width', (
      tester,
    ) async {
      ...
      expect(analytics.buttonEvents.single, {
        'button_id': 'listing_details',
  ...
  for (final inDialog in [false, true]) {
    testWidgets(
      'all info-sheet actions include listing context (inDialog=$inDialog)',
      (tester) async {
  ...
  testWidgets('settings logs taps and the new preference value', (
    tester,
  ) async {
    await tester.pumpWidget(
      MaterialApp(home: SettingsPage(analyticsService: analytics)),
    );
    ...
    expect(analytics.calls, [
      'haptic',
      'tap:distanceUnit_preference_option',
      'unit:imperial',
    ]);
    analytics.calls.clear();
    ...
    expect(analytics.calls, [
      'haptic',
      'tap:theme_preference_option',
      'theme:dark',
    ]);
  });

  testWidgets('analytics explanation lists tracked and untracked data', (
    tester,
  ) async {
    ...
    expect(
      find.text('• Words and phrases you enter in in-app searches.'),
      findsOneWidget,
    );
  ...
  testWidgets(
    'only the visible tab is tracked and returning from Settings restores it',
    (tester) async {
      await tester.pumpWidget(
        MyApp(firstExecution: false, analyticsService: analytics),
      );
      await tester.pumpAndSettle();
      expect(analytics.calls.where((call) => call.startsWith('screen:')), [
        'screen:ChooserPage',
      ]);
